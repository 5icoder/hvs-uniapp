			...
			<view v-else-if="data">
				<uni-table :loading="loading" border stripe type="selection" emptyText="暂无更多数据">
					<uni-tr>
						...
						<uni-th width="150" align="center">表单编号</uni-th>
						<uni-th width="150" align="center">用户编号</uni-th>
						<uni-th align="center">体温（早）</uni-th>
						<uni-th align="center">体温（中）</uni-th>
						<uni-th align="center">体温（晚）</uni-th>
						<uni-th width="204" align="center">健康状态</uni-th>
					</uni-tr>
					<uni-tr v-for="(item, index) in data" :key="index">
						<uni-td align="center">{{ item.record.create_date }}</uni-td>
						<uni-td align="center">{{ item.form_id }}</uni-td>
						<uni-td align="center">
							<view class="name">{{ item.register_id }}</view>
						</uni-td>
						<uni-td align="center">{{ item.record.data[0] }}</uni-td>
						<uni-td align="center">{{ item.record.data[1] }}</uni-td>
						<uni-td align="center">{{ item.record.data[2] }}</uni-td>
...
<script>
	...
	const dbSearchFields = ['register_id', 'form_id', 'record.create_date']
export default {
	data() {
		return {
			options: {
	...
	methods: {
		...
		search(e) {
			let newWhere = ''
			if(!this.getWhere()){
				newWhere = `form_id=='CODE_FORM_001'`
			}else{
				newWhere = `form_id=='CODE_FORM_001'&&(${this.getWhere()})`
			}
		...
		reload() {
			this.query = ''
			this.where = "form_id=='CODE_FORM_001'",
			this.loadData()
		}
	}
}
